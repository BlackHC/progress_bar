...
class LogFriendlyProgressBar(ProgressBar):
    ...
    def update(self, delta_processed=1):
        ...
        while self.num_processed >= self.length * (self.num_finished_sections + 1) / self.num_sections:
            self.num_finished_sections += 1
            cur_time = self.get_time()
            elapsed_time = cur_time - self.start_time

            expected_time = elapsed_time * self.num_sections / self.num_finished_sections
    ...
    def print_header(num_iterations):
        ...
        LogFriendlyProgressBar.print(
            '|' + '|'.join(f'{f"{int((index + 1) * 100 / LogFriendlyProgressBar.num_sections)}%":^11}' for index in
                           range(LogFriendlyProgressBar.num_sections)) + '|')
